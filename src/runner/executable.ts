...
export async function fetchBinary(name: string): Promise<[string, Language, string]> {
    ...
    const isCurrentlyWorking = await fileIsExit(lockFileName);
    // The binary already exists, no need for locking
    if (await fileIsExit(targetName) && !isCurrentlyWorking) {
        ...
        try {
            ...
            if (await fileIsExit(targetName)) {
                winston.debug(`Work ${name} done by others...`);
            } else {
                winston.error(`Doing work: fetching binary for ${name} ...`);
