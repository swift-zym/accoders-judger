...
export async function compile(
    code: string, language: Language, extraFiles: FileContent[] = [], priority: number,use_sanitize:boolean
    ...
    try {
        if (await redis.checkBinaryExistance(fingerprint)) {
            ...
            const task: CompileTask = {
                ...
                use_sanitize: use_sanitize&&language.name.startsWith("cpp")
            };
